...
namespace HarmonyHub
{
    public class Client : IDisposable
    {
        ...
        public string HubProfiles { get { return _hubProfiles; } }
        ...
        public Client(string ip, string username, string password, bool bypassLogitech)
        {
            _ip = ip;
            _username = username;
            _password = password;
           // _bypassLogitech = bypassLogitech;
            _bypassLogitech = true;
        }
                        ...
                        while (true)
                        {
                            ...
                            if (ret.Name == "iq")
                            {
                                if (ret.FirstChild != null && ret.FirstChild.Name == "oa")
                                {
                                    ...
                                    {
                                        int i= ret.OuterXml.IndexOf("[CDATA[");
                                        string S = ret.OuterXml.Substring(i+7);
                                        int t= S.IndexOf("]]");
                                        S = S.Substring(0, t);
                                        string[] T = S.Split(':');
                                        foreach(string TT in T)
                                        {
                                            string[] Q = TT.Split(new char[] {'='}, 2, StringSplitOptions.None);
                                            ...
                                            switch (Q[0])
                                            {
                                                case "hubId":
                                                    ...
                                                    break;
                                                case "hubProfiles":
                                                    _hubProfiles = Q[1].Replace("{","").Replace("}","");
                                                    break;
                                                case "productId":
                                                    _productId = Q[1];
                                                    break;
                                        ...
                                        //    int i = ret.OuterXml.IndexOf("friendlyName");
                                        //if(i>-1)
                                        //{
                                        //    string S= ret.OuterXml.Substring(i).Trim();
                                        //    int t = S.IndexOf("=");
                                        //    if (t > -1)
                                        //        S = S.Substring(t + 1).Trim();
